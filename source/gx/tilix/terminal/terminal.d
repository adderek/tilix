...
private:
    ...
    Widget createVTE() {
        ...
        vte.addOnTilixBell(delegate(string params, VTE term) {
            if (vte.getMapped()) {
                onTilixBell(params);
            }
        });
    ...
    void clearBellPending() {
        if (!_bellPending && !_bellPlayer.isPlaying() && !_lblBellMsg.getVisible() && !spBell.getVisible()) return;
        _bellPending = false;
        if (spBell.getVisible()) {
            spBell.stop();
            spBell.hide();
    ...
    void onTilixBell(string params) {
        ...
        if (doSound) {
            ...
            if (soundFile.length > 0) {
                ...
                // No custom sound file — generate a synthetic tone via GStreamer
                _bellPlayer.playTone();
            }
        }
